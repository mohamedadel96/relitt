        ...
        <div class="post rounded" v-for="(feed,i) in feeds" :key="i">
            <div class="post-info px-3 mt-3 d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center py-2">
                    <img class="border rounded-circle mr-3" :src="feed.user.image" alt="friend profile picture" />
                    <div>
                ...
                <div>
                    <span>
                        <img class="pointer" src="../../../assets/icons/shareIco.svg" style="width:24px ; height:24px" alt />
                    </span>
                </div>
            </div>

            <div class="post-media mb-3">
                <img class="st-pic col-12 px-0" v-if="feed.images.length" :src="feed.images[0].url" alt="post picture" />
                <div class="pics d-flex" v-if="feed.images.length - 1 > 0">
                    <div :class="[feed.images.length - 1 == 1 ? 'col-12' : '', feed.images.length - 1 == 2 ? 'col-6' : '', feed.images.length - 1 > 2 ? 'col-4' : '']">
                        <img class="col-12 px-0" v-if="feed.images[1]" :src="feed.images[1].url" alt="post picture" />
                    </div>

                    <div :class="[feed.images.length - 2 == 1 ? 'col-6' : 'col-4']">
                        <img class="col-12 px-0" v-if="feed.images[2]" :src="feed.images[2].url" alt="post picture" />
                    </div>
                    <div class="col-4">
                        <img class="col-12 px-0" v-if="feed.images[3]" :src="feed.images[3].url" alt="post picture" />
                        <div class="imagesNum fontXXL font-weight-bold" v-if="feed.images.length - 4">+ {{feed.images.length - 4}}</div>
                    </div>
                </div>
            </div>
